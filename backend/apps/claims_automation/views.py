...
def prepare_claims_features(claim):
    ...
    features = {
        ...
        'number_of_injuries': claim.number_of_injuries,
        'third_party_involved': int(claim.third_party_involved),
        'police_report_filed': int(claim.police_report_filed),
...
def estimate_claim_cost(request):
    ...
    try:
        ...
        if data.get('override_injuries') is not None:
            features_df['number_of_injuries'] = data['override_injuries']
        if data.get('override_vehicles') is not None:
            features_df['number_of_vehicles_involved'] = data['override_vehicles']
        ...
        risk_factors = {
            ...
            'injury_count': f"{claim.number_of_injuries} injuries reported",
            'vehicles_involved': f"{claim.number_of_vehicles_involved} vehicles involved",
            'third_party': 'Third party involved' if claim.third_party_involved else 'No third party',
            'police_report': 'Police report filed' if claim.police_report_filed else 'No police report'
        }
...
def processing_recommendations(request, claim_id):
    ...
    try:
        ...
        if claim.number_of_injuries > 0:
            recommendations['actions'].append({
                'action': 'Medical assessment required',
                'reason': f'{claim.number_of_injuries} injuries reported',
                'priority': 'HIGH'
            })
        
        if not claim.police_report_filed and estimate.estimated_cost > 5000:
            recommendations['actions'].append({
                'action': 'Request police report',
                'reason': 'High value claim without police documentation',
                'priority': 'MEDIUM'
            })
        
        # Add reserve recommendation
        recommendations['reserve_action'] = {
            'current_reserve': 0,  # Would come from policy
            'recommended_reserve': float(estimate.recommended_reserve),
...
def estimate_claim_cost_direct(request):
    ...
    try:
        ...
        try:
            ...
            if not claim_id:
                # Adjust based on repair complexity
                ...
                ) + (labor_cost * 0.3)  # Add 30% of labor cost
            ...
            if not claim_id:
                # Rule-based estimation for new claims
                ...
                # Adjust for complexity and parts
                ...
                # For existing claims, use a simple percentage of vehicle value
        ...
        # If existing claim, optionally save the estimate
        if claim_id and not existing_estimate:
            # You can choose to auto-save or require explicit save
            # For now, just return the data without saving
...
def claims_statistics(request):
    ...
    try:
        ...
        # Accuracy statistics (only for estimates with actual settlements)
...
def auto_process_claim(request, claim_id):
    ...
    Returns comprehensive AI analysis for claim processing
    """
    ...
    try:
        ...
        # Get fraud analysis (this returns a Response object, so we need to extract data)
        ...
        try:
            ...
            if not estimate:
                ...
                # Determine recommendation based on cost AND fraud score
            ...
            # Continue without estimate if it fails
            estimate_data = {
                'error': 'Cost estimation unavailable',
                'message': str(e)
            }
        
        # STEP 3: Generate Combined Processing Recommendations
        fraud_score = float(fraud_data.get('fraud_score', 0))
        ...
        if not claim.police_report_filed and estimated_cost > 5000:
            action_items.append({
                'action': 'Request Police Report',
                'priority': 'MEDIUM',
                'reason': 'High value claim without documentation'
            })
        ...
        response_data = {
            ...
            # Include full details for reference
